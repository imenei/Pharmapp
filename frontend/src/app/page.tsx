...
const Icons = {
  Logo: () => (
    <svg viewBox="0 0 40 40" fill="none" className="w-full h-full">
      ...
      <path d="M20 8C13.373 8 8 13.373 8 20s5.373 12 12 12 12-5.373 12-12S26.627 8 20 8zm0 4a8 8 0 1 1 0 16 8 8 0 0 1 0-16zm-1 3v5h-5v2h5v5h2v-5h5v-2h-5v-5h-2z" fill="white"/>
    </svg>
  ),
  Menu: () => (
  ...
  MapPin: () => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5 inline mr-1 flex-shrink-0">
  ...
  Phone: () => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 mr-2 flex-shrink-0">
...
export default function LandingPage() {
  ...
  const pharmacistFeatures = [
    { icon: <Icons.Search />, t: 'Recherche avancée', d: 'Trouvez des produits par wilaya, nom et type.' },
  ...
  const supplierFeatures = [
    { icon: <Icons.BarChart />, t: 'Statistiques détaillées', d: 'Suivez les performances de vos listings.' },
    { icon: <Icons.Upload />, t: 'Gestion des listings', d: 'Upload, modification et suppression de vos PDF.' },
    { icon: <Icons.Gift />, t: 'Offres promotionnelles', d: 'Publiez des offres spéciales avec images.' },
    { icon: <Icons.Crown />, t: 'Abonnements premium', d: 'Maximisez votre visibilité avec nos plans.' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2.5 flex-shrink-0">
              <div className="w-9 h-9"><Icons.Logo /></div>
              <span className="text-base font-bold text-gray-900 tracking-tight">ELSAIDALIYA</span>
            </div>

            {/* Nav centre desktop */}
            <nav className="hidden md:flex items-center gap-8">
              <a href="#features" className="text-sm text-gray-600 hover:text-[#2E7D32] font-medium transition-colors">Fonctionnalités</a>
              <a href="#pricing"  className="text-sm text-gray-600 hover:text-[#2E7D32] font-medium transition-colors">Abonnements</a>
              <a href="#about"    className="text-sm text-gray-600 hover:text-[#2E7D32] font-medium transition-colors">À propos</a>
              <Link href="/contact" className="text-sm text-gray-600 hover:text-[#2E7D32] font-medium transition-colors">Contact</Link>
            </nav>

            {/* Connexion + Inscription droite desktop */}
            <div className="hidden md:flex items-center gap-3">
              <Link href="/auth/signin" className="text-sm text-gray-700 font-medium hover:text-[#2E7D32] transition-colors px-2">
                Connexion
              </Link>
              <Link href="/auth/signup" className="bg-[#2E7D32] text-white text-sm font-semibold px-5 py-2 rounded-lg hover:bg-[#1B5E20] transition-colors">
                Inscription
              </Link>
            </div>
            ...
            <div className="md:hidden py-4 border-t border-gray-100">
              <div className="flex flex-col gap-3">
                {[['#features','Fonctionnalités'],['#pricing','Abonnements'],['#about','À propos']].map(([href,label])=>(
                  <a key={href} href={href} onClick={()=>setMenuOpen(false)} className="text-sm text-gray-600 hover:text-[#2E7D32] font-medium py-1">{label}</a>
                ))}
                <Link href="/contact" onClick={()=>setMenuOpen(false)} className="text-sm text-gray-600 hover:text-[#2E7D32] font-medium py-1">Contact</Link>
                <div className="border-t border-gray-100 pt-3 flex flex-col gap-2">
                  <Link href="/auth/signin" onClick={()=>setMenuOpen(false)} className="text-sm text-gray-700 font-medium text-center py-2 hover:text-[#2E7D32]">Connexion</Link>
                  <Link href="/auth/signup" onClick={()=>setMenuOpen(false)} className="bg-[#2E7D32] text-white text-sm font-semibold px-5 py-2.5 rounded-lg text-center">Inscription</Link>
                </div>
              </div>
            </div>
          )}
      ...
      {/* ── HERO ── */}
      <section className="relative bg-[#E8F5E9] overflow-hidden min-h-[88vh] flex items-center">
        {/* Cercles décoratifs */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-[8%]   right-[3%]  w-72 h-72 rounded-full bg-[#C8E6C9] opacity-50"/>
          <div className="absolute top-[40%]  right-[22%] w-44 h-44 rounded-full bg-[#A5D6A7] opacity-35"/>
          <div className="absolute bottom-[8%] right-[6%]  w-56 h-56 rounded-full bg-[#C8E6C9] opacity-30"/>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-16 md:py-0">
          <div className="flex flex-col md:flex-row items-center">
            ...
            <div className="md:w-1/2 md:pr-12">
              <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-5 leading-[1.15] tracking-tight">
                La plateforme{' '}
                <span className="text-[#2E7D32]">pharmaceutique</span>{' '}
                algérienne nouvelle génération
              </h1>
              <p className="text-base text-gray-500 mb-8 leading-relaxed max-w-md">
                Connectez pharmaciens et fournisseurs dans un écosystème sécurisé et professionnel. Optimisez votre chaîne d&apos;approvisionnement avec des outils modernes.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                <Link href="/auth/signup" className="bg-[#2E7D32] text-white px-7 py-3 rounded-lg hover:bg-[#1B5E20] transition-colors text-sm font-bold text-center shadow-sm">
                  Commencer en tant que Pharmacien
                </Link>
                <Link href="/auth/signup" className="bg-white text-gray-700 border border-gray-200 px-7 py-3 rounded-lg hover:border-[#2E7D32] hover:text-[#2E7D32] transition-colors text-sm font-semibold text-center">
                  Devenir Fournisseur
                </Link>
              </div>
            </div>

            {/* ✅ FIX : mix-blend-multiply supprime le fond coloré de l'image */}
            <div className="md:w-1/2 flex justify-center md:justify-end mt-10 md:mt-0">
              <img
                src="/page-destination-pharmacie.png"
                alt="Pharmacien avec médicaments"
                className="w-full max-w-xs md:max-w-sm lg:max-w-md object-contain select-none mix-blend-multiply"
                draggable={false}
              />
            </div>
          </div>
      ...
      {/* STATS BAR */}
      <section className="bg-[#2E7D32] py-7">
      ...
      <section id="features" className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-500 max-w-xl mx-auto text-sm">Découvrez comment ELSAIDALIYA transforme l&apos;approvisionnement pharmaceutique</p>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-4 border border-amber-100">
              <Icons.Crown /><span className="ml-1">Abonnement Or</span>
            </div>
          ...
          {suppLoading?(
                ...
                <div key={s.id} className="bg-white rounded-xl border border-gray-100 p-6 hover:shadow-md transition-all hover:-translate-y-0.5">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="min-w-0">
                      ...
                      {s.wilaya&&<p className="text-xs text-gray-400 mt-0.5 flex items-center"><Icons.MapPin/>{s.wilaya}</p>}
                    </div>
            ...
            <div className="text-center py-16">
              <div className="flex justify-center mb-4"><Icons.Trophy/></div>
      ...
      <section id="pricing" className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-sm transition-all">
              <div className="flex items-center gap-2 mb-4">
                <Icons.Medal color="text-orange-400"/>
              ...
              <Link href="/auth/signup" className="block w-full text-center bg-gray-50 text-gray-700 border border-gray-100 py-2.5 rounded-lg hover:bg-gray-100 transition-colors text-sm font-semibold">Choisir Bronze</Link>
            </div>
            <div className="bg-[#2E7D32] rounded-2xl p-6 relative shadow-lg">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-400 text-white text-xs font-bold px-4 py-1 rounded-full whitespace-nowrap">PLUS POPULAIRE</div>
              <div className="flex items-center gap-2 mb-4">
                <Icons.Medal color="text-gray-300"/>
              ...
              <Link href="/auth/signup" className="block w-full text-center bg-white text-[#2E7D32] py-2.5 rounded-lg hover:bg-green-50 transition-colors text-sm font-bold">Choisir Argent</Link>
            </div>
            <div className="bg-white border border-amber-100 rounded-2xl p-6 hover:shadow-sm transition-all">
              <div className="flex items-center gap-2 mb-4">
                <Icons.Medal color="text-amber-500"/>
              ...
              <Link href="/auth/signup" className="block w-full text-center bg-amber-50 text-amber-700 border border-amber-100 py-2.5 rounded-lg hover:bg-amber-500 hover:text-white hover:border-amber-500 transition-colors text-sm font-semibold">Choisir Or</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="about" className="py-20 bg-gray-50 border-t border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              {icon:<Icons.Shield/>,t:'Sécurisé',d:'Authentification multi-niveaux et données cryptées.'},
              {icon:<Icons.Zap/>,t:'Rapide',d:'Interface optimisée pour une expérience fluide et réactive.'},
              {icon:<Icons.Monitor/>,t:'Moderne',d:"Design responsive qui s'adapte à tous les appareils."},
            ].map(({icon,t,d})=>(
              <div key={t} className="bg-white p-6 rounded-xl border border-gray-100 text-center hover:shadow-sm transition-all">
                <div className="w-14 h-14 bg-[#E8F5E9] text-[#2E7D32] rounded-xl flex items-center justify-center mx-auto mb-4">{icon}</div>
      ...
      <section className="py-20 bg-[#1B5E20]">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <Link href="/auth/signup" className="bg-white text-[#2E7D32] px-6 py-3 rounded-lg hover:bg-green-50 transition-colors text-sm font-semibold shadow-sm">Créer un compte gratuit</Link>
            <Link href="/auth/signin" className="border border-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors text-sm font-semibold">Se connecter</Link>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-white border-t border-gray-100 py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-bold text-gray-900 tracking-tight text-sm">ELSAIDALIYA</span>
              </div>
          ...
          <div className="pt-6 border-t border-gray-50 flex flex-col md:flex-row justify-between items-center gap-2">
            <p className="text-xs text-gray-300">© 2025 Elsaidaliya. Tous droits réservés.</p>
