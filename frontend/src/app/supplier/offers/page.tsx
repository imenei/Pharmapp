...
import { Plus, Trash2, Gift, Eye } from 'lucide-react';
import { useMyOffers, useCreateOffer, useDeleteOffer } from '@/hooks/useApi';
...
export default function SupplierOffersPage() {
  ...
  const handleCreate = async () => {
    if (!form.title || !form.description || !form.expiresAt) { setError('Tous les champs sont requis'); return; }
    setError('');
    ...
    try {
      ...
      setModal(false); setForm({ title: '', description: '', expiresAt: '' }); setImage(null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModal(true)} className="btn-primary"><Plus size={16}/> Créer une offre</button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-16">
          ...
          <p className="font-semibold text-gray-600 mb-1">Aucune offre créée</p>
          <p className="text-sm text-gray-400 mb-6">Créez votre première offre promotionnelle</p>
          <button onClick={() => setModal(true)} className="btn-primary mx-auto"><Plus size={16}/> Créer une offre</button>
        </div>
      ) : (
          ...
          {offers.map((o: any) => {
            ...
            return (
              <div key={o.id} className={`card flex flex-col ${expired ? 'opacity-60' : ''}`}>
                {o.imageUrl && (
                  <img src={`${API_BASE}${o.imageUrl}`} alt={o.title}
                    className="w-full h-36 object-cover rounded-lg mb-3"/>
                )}
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-semibold text-gray-900 line-clamp-2 flex-1">{o.title}</h3>
                  <button onClick={() => confirm('Supprimer cette offre ?') && remove.mutate(o.id)}
      ...
      <Modal open={modal} onClose={() => { setModal(false); setError(''); }} title="Créer une offre promotionnelle">
        <div className="space-y-4">
          <div>
            <label className="label">Titre *</label>
            <input className="input" placeholder="Ex: Promotion antibiotiques -20%"
              value={form.title} onChange={e => setForm(p=>({...p, title: e.target.value}))}/>
          </div>
          <div>
            <label className="label">Description *</label>
            <textarea className="input h-28 resize-none" placeholder="Décrivez votre offre en détail…"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div onClick={() => imgRef.current?.click()}
                className="input flex items-center gap-2 cursor-pointer hover:bg-gray-50 text-gray-500">
